...
class IdentityTests(unittest.TestCase):
    ...
    def test_pf_identity_manage_admin_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)

            response = c.get('/identity/manage')
    ...
    def test_pf_identity_manage_admin_perms_no_settings(self):
        pitchfork.db.api_settings.remove()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_manage_user_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)

            result = c.get('/identity/manage')
    ...
    def test_pf_identity_manage_add_update(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
        ...
        cloud_identity = api_settings.get('cloud_identity')
        updated = False
        if cloud_identity.get('title') == 'Test':
            updated = True

        assert updated, 'Item was not updated successfully'
        self.teardown_app_data()

    def test_pf_identity_manage_add_update_disable(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
        ...
        not_active = False
        if not 'cloud_identity' in active_products:
            not_active = True

        assert not_active, 'Product was not removed from active products'
        self.teardown_app_data()

    def test_pf_identity_manage_add_bad_data(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_manage_api_admin_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)

            response = c.get('/identity/manage/api')
    ...
    def test_pf_identity_manage_api_user_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)

            result = c.get('/identity/manage/api')
    ...
    def test_pf_identity_manage_api_add_admin_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)

            response = c.get('/identity/manage/api/add')
    ...
    def test_pf_identity_manage_api_add_user_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)

            result = c.get('/identity/manage/api/add')
    ...
    def test_pf_identity_manage_api_add_admin_post_dupe_title(self):
    ...
    def test_pf_identity_manage_api_add_admin_post_dupe_url(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_manage_api_add_admin_post_good(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
        ...
        found_call = pitchfork.db.cloud_identity.find_one(
            {
                'title': 'Add Call'
            }
        )
    ...
    def test_pf_identity_manage_api_edit_user_perms(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)
            ...
            result = c.get(
                '/identity/manage/api/edit/%s' % api_id
            )
    ...
    def test_pf_identity_manage_api_edit_admin_perms(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_manage_api_edit_admin_perms_with_vars(self):
    ...
    def test_pf_identity_manage_api_edit_admin_bad_post(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_manage_api_edit_admin_good_post(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
        ...
        calls = pitchfork.db.cloud_identity.find_one(
            {
                'title': 'Test Update Call'
            }
        )
    ...
    def test_pf_identity_manage_api_mark_tested(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
            ...
            response = c.get(
                '/identity/manage/api/test/confirm/%s' % api_id,
                follow_redirects=True
            )
        ...
        api_call = pitchfork.db.cloud_identity.find_one()
        tested = api_call.get('tested')
        assert tested, 'API call was not saved as tested'
        self.teardown_app_data()

    def test_pf_identity_manage_api_mark_untested(self):
        ...
        with pitchfork.app.test_client() as c:
            ...
            response = c.get(
                '/identity/manage/api/test/unconfirm/%s' % api_id,
                follow_redirects=True
            )
    ...
    def test_pf_identity_manage_api_delete_valid(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
            ...
            response = c.get(
                '/identity/manage/api/delete/%s' % api_id,
                follow_redirects=True
            )
        ...
        api_call = pitchfork.db.cloud_identity.find()
        self.assertEquals(
            api_call.count(),
            0,
    ...
    def test_pf_identity_manage_api_delete_invalid(self):
        api_id = self.setup_useable_api_call()
        ...
        with pitchfork.app.test_client() as c:
            ...
            response = c.get(
                '/identity/manage/api/delete/%s' % bogus_id,
                follow_redirects=True
            )
    ...
    def test_pf_identity_api_admin_perms_testing(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)

            response = c.get('/identity/testing')

        self.assertIn(
            '<h3>Cloud Identity - Testing API Calls</h3>',
            response.data,
            'Did not find correct HTML on page'
        )
        self.teardown_app_data()

    def test_pf_identity_api_admin_perms_testing_no_settings(self):
        pitchfork.db.api_settings.remove()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_api_user_perms_testing(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)
    ...
    def test_pf_identity_api_admin_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)

            response = c.get('/identity/')
    ...
    def test_pf_identity_api_user_perms(self):
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)
    ...
    def test_pf_identity_api_admin_perms_no_settings(self):
        pitchfork.db.api_settings.remove()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_api_user_perms_no_settings(self):
        pitchfork.db.api_settings.remove()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_user_login(sess)
    ...
    def test_pf_identity_post_call(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_post_call_testing(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_post_call_testing_uk(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_post_call_bad_response(self):
        api_id = self.setup_useable_api_call()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
    ...
    def test_pf_identity_post_call_with_data(self):
        api_id = self.setup_useable_api_call_with_variables()
        with pitchfork.app.test_client() as c:
            with c.session_transaction() as sess:
                self.setup_admin_login(sess)
